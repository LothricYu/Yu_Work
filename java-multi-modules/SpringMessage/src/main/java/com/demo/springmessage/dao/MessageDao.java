...
public interface MessageDao {
    @Select("select m_id,m_title,m_detail,starttime,edittime,userid,username from message inner join user on userid = id")
    ...
    //分页查询
    @Select("select m_id,m_title,m_detail,starttime,edittime,userid,username from message inner join user on userid = id limit #{begin},#{end}")
    ...
    @Select("select m_id,m_title,m_detail,starttime,edittime,userid,username from message inner join user on userid = id where m_id=#{message_id}")
    @ResultMap("messageResultMap")
    Message selectMessageById(int message_id);

    @Select("select m_id,m_title,m_detail,starttime,edittime,userid,username from message inner join user on userid = id where user.username=#{username}")
    @ResultMap("messageResultMap")
