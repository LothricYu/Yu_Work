...
<script type="text/javascript">
    function diag(messageid)
    {
        var activity = window.prompt('请输入打赏金额：');
        if(activity!="")
        {
            window.location.href = 'message_reward?reward_id='+messageid+'&userid=<%=user.getId()%>&number=' + window.encodeURIComponent(activity);
        }
        else{
            alert("输入为空");
        }
    ...
    <c:forEach items="<%=messageList%>" var="message">
        <tr>
            ...
            <td><a href="message_delete?deleteid=${message.id}&userid=${message.userId}">删除</a>
                ...
                <input type="button" name="dashang" value="打赏" onclick="diag(${message.userId})">
            </td>
        </tr>
    </c:forEach>
    </tbody>
