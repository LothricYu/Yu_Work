...
public interface UserDao {
    ...
    @Select("select username,id,username,password,email,telnumber,photo,balance,groupid,roleid,(select count(*) from message where userid=id) as count from user where username=#{username}")
    ...
    @Update("update user set balance=#{number} where id=#{id}")
    int updateUserBalance(Double number,int id);

    //减少余额
    @Update("update user set balance=balance-#{number} where id=#{id}")
    int reduceUserBalance(Double number,int id);

    //添加余额
    @Update("update user set balance=balance+#{number} where id=#{id}")
    int addUserBalance(Double number,int id);
